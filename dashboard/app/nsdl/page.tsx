...
// MF reconciliation — match NSDL CAS mutual funds against alphaos
// (Supabase-backed /api/mutual-funds). Names are messy ("Mirae Asset Large
// Cap Fund - Direct Plan" vs "Mirae Asset Large Cap Direct"), so match on
// the first 3 normalised tokens.
interface AlphaosMfRow { scheme_name: string; value: number | null; isin: string | null }

function normaliseMfName(s: string): string[] {
  return s
    .toLowerCase()
    .replace(/\bdirect\b|\bplan\b|\bgrowth\b|\bregular\b|\boption\b|\bidcw\b|\bfund\b/g, '')
    .replace(/[.,&\-]/g, ' ')
    .split(/\s+/)
    .filter(t => t.length >= 3);
}

function reconcileMfs(
  holdings: NsdlHolding[],
  alphaosMfs: AlphaosMfRow[],
): { matched: { holding: NsdlHolding; mf: AlphaosMfRow }[]; onlyInNsdl: NsdlHolding[]; onlyInAlphaos: AlphaosMfRow[] } {
  const nsdlMfs = holdings.filter(h => h.type === 'mutual_fund');
  const matched: { holding: NsdlHolding; mf: AlphaosMfRow }[] = [];
  const used = new Set<string>();
  const unmatched: NsdlHolding[] = [];

  for (const h of nsdlMfs) {
    const nsdlTokens = normaliseMfName(h.name);
    if (nsdlTokens.length === 0) { unmatched.push(h); continue; }
    const mf = alphaosMfs.find(m => {
      if (used.has(m.scheme_name)) return false;
      const mfTokens = normaliseMfName(m.scheme_name);
      // Require at least 2 token-prefix matches (any order) for a confident match
      let hits = 0;
      for (const t of nsdlTokens.slice(0, 3)) {
        if (mfTokens.some(mt => mt.startsWith(t.slice(0, 4)) || t.startsWith(mt.slice(0, 4)))) hits++;
      }
      return hits >= 2;
    });
    if (mf) {
      matched.push({ holding: h, mf });
      used.add(mf.scheme_name);
    } else {
      unmatched.push(h);
    }
  }

  const onlyInAlphaos = alphaosMfs.filter(m => !used.has(m.scheme_name));
  return { matched, onlyInNsdl: unmatched, onlyInAlphaos };
}
...
export default function NsdlPage() {
  ...
  const { data: mfData } = useQuery<{ funds: AlphaosMfRow[] }>({
    queryKey: ['mf-for-nsdl'],
    queryFn: async () => {
  ...
  const mfRecon = useMemo(() => {
    if (!nsdl?.latestHoldings?.length || !mfData?.funds?.length) return null;
    return reconcileMfs(nsdl.latestHoldings, mfData.funds);
  }, [nsdl, mfData]);
        ...
        <Card className="bg-card border-border">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
              <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 p-3">
                <div className="text-xs text-emerald-400 font-700">✅ In both</div>
                <div className="text-xl font-800 mt-1">{mfRecon.matched.length}</div>
              </div>
              <div className="rounded-lg border border-yellow-500/20 bg-yellow-500/5 p-3">
                <div className="text-xs text-yellow-400 font-700">🟡 In NSDL only</div>
                <div className="text-xl font-800 mt-1">{mfRecon.onlyInNsdl.length}</div>
                <div className="text-[10px] text-muted-foreground">upload CAS via dashboard to import</div>
              </div>
              <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-3">
                <div className="text-xs text-red-400 font-700">🔴 In alphaos only</div>
                <div className="text-xl font-800 mt-1">{mfRecon.onlyInAlphaos.length}</div>
                <div className="text-[10px] text-muted-foreground">not in latest CAS — redeemed?</div>
              </div>
            </div>

            {mfRecon.onlyInNsdl.length > 0 && (
              <div className="mb-3">
                <div className="text-xs font-600 text-muted-foreground mb-1">In NSDL but missing from alphaos:</div>
                <div className="flex flex-wrap gap-1.5">
                  {mfRecon.onlyInNsdl.map(h => (
                    <span key={h.name} className="text-xs bg-yellow-500/10 text-yellow-300 border border-yellow-500/20 rounded px-2 py-0.5">
                      {h.name.replace(/ - Direct.*$|Direct.*$/i, '')} <span className="text-yellow-300/60">· {formatCr(h.value)}</span>
                    </span>
                  ))}
                </div>
              </div>
            ...
            {mfRecon.onlyInAlphaos.length > 0 && (
              <div>
                <div className="text-xs font-600 text-muted-foreground mb-1">In alphaos but not in latest CAS:</div>
                <div className="flex flex-wrap gap-1.5">
                  {mfRecon.onlyInAlphaos.map(m => (
                    <span key={m.scheme_name} className="text-xs bg-red-500/10 text-red-300 border border-red-500/20 rounded px-2 py-0.5">
                      {m.scheme_name.replace(/ - Direct.*$|Direct.*$/i, '')}
                    </span>
                  ))}
                </div>
              </div>
        ...
        <Card className="bg-card border-dashed border-border">
          <CardContent className="p-6 text-center">
            ...
            <div className="text-xs text-muted-foreground mt-1 max-w-lg mx-auto">
              The <code className="text-[11px] bg-white/5 px-1 rounded">nsdl_holdings</code> tab in the India sheet is empty.
              Run <code className="text-[11px] bg-white/5 px-1 rounded">python nsdl_cas/scripts/sync_holdings_to_sheet.py</code> locally
              to push the latest snapshot, then refresh.
            </div>
          </CardContent>
        </Card>
      )}
