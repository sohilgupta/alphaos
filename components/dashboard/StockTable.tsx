      ...
      <button type="button" onClick={onOpen} className="w-full text-left p-4 active:opacity-80">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <p className="text-sm font-700 text-foreground truncate">{stock.live?.shortName || stock.name}</p>
            <div className="mt-0.5 flex items-center gap-1.5 flex-wrap">
              <span className="text-xs font-600 text-muted-foreground">{stock.ticker}</span>
          ...
          {sorted.map(stock => (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="text-sm font-700 text-foreground group-hover:text-primary transition-colors">{stock.ticker}</div>
                  <div className="text-xs text-muted-foreground truncate max-w-[100px]">{stock.live?.shortName || stock.name}</div>
                </div>
                  ...
                  {sorted.map(stock => {
                    ...
                    return (
                      <tr key={stock.ticker} onClick={() => router.push(`/stock/${stock.ticker}`)} className="ticker-row group">
                        ...
                        <td className="px-3 py-2.5">
                          <span className="font-700 text-foreground group-hover:text-primary transition-colors">{stock.ticker}</span>
                        </td>
                        <td className="px-3 py-2.5">
